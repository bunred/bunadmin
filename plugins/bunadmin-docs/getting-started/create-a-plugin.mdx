...
*Folders starting with `bunadmin-plugin-` in the `plugins/` directory will be automatically recognized.*
...
Replace the content of `utils/initData.ts` with the following:
``` ts
...
Head back to [http://localhost:1911](http://localhost:1911), refresh the page and click the `Post` menu.
...
*If you see the screenshot page, it means that the plugin has been created and successfully activated, otherwise please check the steps above.*
